...
//	the number of food for each Philosopher
    ...
    public Main(int i, Semaphore f1, Semaphore f2)
    {
        FirstFork = f1;
        SecondFork = f2;
        id = i;
        ...
        while (FoodAmount > 0)
        {
            ...
            try {
                ...
                if (!SecondFork.tryAcquire()) {
                	System.out.printf("#%d Forks left; Philosopher %dfailed to pick second fork \n", remainedFork, id);
                    return;
                };
    ...
    public static void main(String[] args)
    {
        ...
        Main[] philosopher = new Main[N];
        for (int m = 0; m < N; m++) {
            int mn = m + 1;
            if (mn == N) mn = 0;
            philosopher[m] = new Main(m, fork[m], fork[mn]); 
        }
        for (int i = 0; i < N; i++) {
              philosopher[i].start();
        }
